...
- 2026-09-25 — **REST API module status (researched, unresolved upstream).** The compendium endpoint our pinned MCP calls (`/api/compendium/search`) was never shipped in any module release: module assets stop at `v0.11.0` (2025-08, serves the four diagnostics endpoints only); every v1.x release carries no module assets; the module was extracted to a standalone repo (PR #84) that is not publicly findable. Options: (a) maintain a local MIT patch of the v0.11.0 module adding `/api/compendium/search`; (b) import adventure compendium content into the world once and read it via the socket path (already working). Decide at next session.
